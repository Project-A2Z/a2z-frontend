...
const Reviews: React.FC<Props> = ({ productId, onReviewAdded }) => {
  ...
  const handleAddReview = async () => {
    if (!authToken) { setError(t("errors.loginRequiredAdd")); return; }
    if (hasUserReview) { setError(t("errors.alreadyReviewed")); return; }
    if (!newReview.description?.trim() || submitting) return;
    ...
    try {
      ...
      const msg = err.message || t("errors.failedAdd");
      if (msg.includes("409") || msg.includes("بالفعل")) {
        setError(t("errors.alreadyReviewed"));
        setHasUserReview(true);
      } else if (msg.includes("انتهت صلاحية") || msg.includes("401")) {
        setError(t("errors.sessionExpired"));
        setAuthToken(null);
        setUserId(null);
      } else if (msg.includes("البيانات") || msg.includes("400")) {
  ...
  const handleEditReview = (review: Review) => {
    if (!isUserReview(review)) { setError(t("errors.cannotEditOthers")); return; }
    if (submitting) { setError(t("errors.pleaseWait")); return; }
    setEditingId(review._id);
    setEditForm({ description: review.description || "", rateNum: review.rateNum });
    setError(null);
  };

  const handleUpdateReview = async () => {
    if (!authToken) { setError(t("errors.loginRequiredEdit")); return; }
    if (!editForm.description?.trim() || !editingId || submitting) return;

    const reviewToUpdate = reviews.find((r) => r._id === editingId);
    if (!reviewToUpdate || !isUserReview(reviewToUpdate)) {
      setError(t("errors.cannotEditOthers"));
      return;
    }
    ...
    try {
      ...
      const msg = err.message || t("errors.failedUpdate");
      if (msg.includes("لم يتم العثور") || msg.includes("404")) {
        setError(t("errors.reviewNotFound"));
        setEditingId(null);
        setEditForm({ description: "", rateNum: 5 });
        await refreshReviews();
      } else if (msg.includes("غير مسموح") || msg.includes("403")) {
        setError(t("errors.noPermissionEdit"));
        setEditingId(null);
        setEditForm({ description: "", rateNum: 5 });
      } else if (msg.includes("انتهت صلاحية") || msg.includes("401")) {
        setError(t("errors.sessionExpired"));
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      const msg = err.message || t("errors.failedDelete");
      if (msg.includes("غير موجود") || msg.includes("تم حذفه") || msg.includes("404")) {
        setError(t("errors.reviewNotFoundDelete"));
      } else if (msg.includes("غير مسموح") || msg.includes("403")) {
        setError(t("errors.noPermissionDelete"));
      } else if (msg.includes("انتهت صلاحية") || msg.includes("401")) {
        setError(t("errors.sessionExpired"));
        setAuthToken(null);
        setUserId(null);
      ...
      <section className="bg-white rounded-2xl border shadow-sm p-4 sm:p-6">
        <div className="text-red-600 text-center py-4">
          ...
          <div className="mt-3 flex justify-center gap-2">
            <button
              onClick={() => { setError(null); refreshReviews(); }}
  ...
  // ── Main render ────────────────────────────────────────────────────────────
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6" style={{direction: isRTL ? 'rtl' : 'ltr' , textAlign : isRTL ? 'right' : 'left'}}>
      {/* Reviews list */}
      <section className="bg-white rounded-lg md:rounded-2xl border shadow-sm p-3 sm:p-4 md:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4">
          <h2 className="sm:text-lg md:text-xl font-bold text-black87" style={{textAlign : isRTL ? 'right' : 'left'}}>
            {t("sectionTitle")}
          </h2>
        ...
        <div className="space-y-3 sm:space-y-4 max-h-[400px] sm:max-h-[500px] overflow-y-auto pr-1">
          {reviews.length === 0 ? (
            <div className="text-center py-6 sm:py-8 text-gray-500">
              ...
              <p className="text-sm sm:text-base">{t("noReviews")}</p>
              <p className="text-xs sm:text-sm mt-1">{t("beFirst")}</p>
            </div>
          ) : (
            reviews.map((review) => (
              <article
                ...
                <div
                  className={`space-y-2 sm:space-y-3 ${
                    editingId === review._id
                      ? "bg-blue-50 p-2 sm:p-3 rounded-lg border-2 border-blue-300 relative"
                      : ""
                  }`}
                >
                  {editingId === review._id && (
                  ...
                  {review.reply && (
                    <div className="mt-2 sm:mt-3 p-2 sm:p-3 bg-primary/10 border-r-4 border-primary rounded-l-lg">
                      ...
                      <p className="text-sm sm:text-base text-primary/80 break-words">
                        {review.reply}
                      </p>
                    </div>
                  )}
      ...
      {/* Add / edit review */}
      <section className="bg-white rounded-lg md:rounded-2xl border shadow-sm p-3 sm:p-4 md:p-6">
        <h2 className=" sm:text-lg font-bold mb-3 sm:mb-4 " style={{textAlign : isRTL ? 'right' : 'left'}}>
        ...
        {authToken ? (
          hasUserReview && !editingId ? (
            <div className="text-green-700 text-center bg-green-50 border border-green-200 rounded-lg p-3 sm:p-4 mb-3 sm:mb-4">
              <p className="mb-2 sm:mb-3 text-xs sm:text-sm">{t("alreadyReviewed")}</p>
              <button
                onClick={() => {
            ...
            <div className="bg-gray-50 p-3 sm:p-4 rounded-lg border">
              <div className="mb-3 sm:mb-4">
                <label className="block text-xs sm:text-sm font-medium mb-2" style={{textAlign : isRTL ? 'right' : 'left'}}>
                  {t("commentLabel")}
                </label>
                <textarea
        ...
        <Alert
          ...
          confirmText={t("errorAlert.confirm")}
          cancelText={t("errorAlert.cancel")}
        />
      </section>
    </div>
  );
